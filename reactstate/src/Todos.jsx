...
export default function Todos() {
  // const todos = todoList.map((x) => {
  //   if (x.completed) {
  //     return (
  //       <li key={x.id}>
  //         <strike>{x.todo}</strike>
  //       </li>
  //     );
  //   }
  ...
  if (showCompleted) {
    todos = todoList.filter((x) => x.completed);
  }
  ...
  return (
    <>
      ...
      <p>
        <label>
          <input type="checkbox" onClick={() => setCompleted(!showCompleted)} />
          Sadece tamamlananlari goster
        </label>
      </p>
      <ul>{todos}</ul>
    </>
