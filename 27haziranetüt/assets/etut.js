...
if (localStorage.movies) {
  //local storage'ta movie varsa
  movies = JSON.parse(localStorage.movies);
  renderMovies();
}

if (localStorage.id) {
  //local storage'ta id varsa
...
function handleMovieForm() {
  ...
  if (formObj.id !== "") {
    ...
    formObj.id = generateId(); // form objede id boşsa yeni bir id oluşturması için generate id fonksiyonu çağırıyoruz
    movies.push(formObj);
  }
  save();
  renderMovies();
...
function save() {
  localStorage.movies = JSON.stringify(movies); //localstorage'a gönderirken string gönderiyoruz çekerken parse ediyoruz
}
...
function handleDeleteBtn(e) {
  // html'de a etiketi kullandığımız için a etiketinin default özelliği çalışmasın diye
  e.preventDefault();

  if (!confirm("Emin misin?")) {
    //kullanıcıya emin misin diye onay mesajı gönder (emin değilse)
    return; //
  }
  movies = movies.filter((x) => x.id !== Number(this.dataset.movieid));
  // this ile tıklananın verilerini alıyoruz. dataset ile tıklananın id'sini yakalıyoruz. eğer tıklanan ile aynı id'ye
  //sahipse onu filtrele diyoruz
  //this.dataset.movieid ile movies içinndeki idleri karşılaştırıyoruz
  // filter koşulu sağlayan tüm verileri getirir
  // find bulduğu ilk koşulu sağlayan ögeyi getirir
  save();
  renderMovies();
}
...
function renderMovies() {
  ...
  // join ile tek bir string ifade haline getirm,ş oluyoruz
  document
